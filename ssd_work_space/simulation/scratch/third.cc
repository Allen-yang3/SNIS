...
std::unordered_map<uint32_t, unordered_map<uint32_t, uint32_t> > portNumder;
...
uint32_t flow_num;
double first_flow_start_time=0;//added by bo
...
void ScheduleFlowInputs(){
        //std::cout<<"schedule flow begin ... ";
	while (flow_input.idx < flow_num){//modified by bo// && Seconds(flow_input.start_time) == Simulator::Now()){
		...
		//if(flow_input.start_time>1)
		//        appCon.Start(Seconds(flow_input.start_time-1));
		//else //modified by bo Time(0)
		appCon.Start(Seconds(flow_input.start_time));//modified by bo Time(0)

                printf("%u,%08x,%08x,%u\n", flow_input.idx, serverAddress[flow_input.src], serverAddress[flow_input.dst], port); //added by bo
                //std::cout << flow_input.idx << " " << serverAddress[flow_input.src]<<" "<<port << std::endl; //added by bo 5/3 for print out flow_ap to queue.txt
...
int main(int argc, char *argv[])
{
	...
	for (uint32_t i = 0; i < node_num; i++){
		if (n.Get(i)->GetNodeType() == 1){ // is switch
			...
			for (uint32_t j = 1; j < sw->GetNDevices(); j++){
				...
				//Lines 837-838 Edited by Allen**
				//uint32_t headroom = rate * delay / 8 / 1000000000 * 3;
				uint32_t headroom = 120000;
	...
	for (uint32_t i = 0; i < node_num; i++){
		if (n.Get(i)->GetNodeType() == 0)
			for (uint32_t j = 0; j < node_num; j++){
				if (n.Get(j)->GetNodeType() == 0)
					portNumder[i][j] = 10000;//(i*256+j)*10000;//10000; modified by bo to make to port number unique ID// each host pair use port number from 10000
			}
	}

	flow_input.idx = 0;
	std::cout << "Flow_ID,Src,Dst,Port"  << std::endl; //added by bo 5/3 for pring out flow_ap to queue.txt
	
	if (flow_num > 0){
		ReadFlowInput();
		first_flow_start_time = flow_input.start_time; //added by bo
		//Simulator::Schedule(Seconds(flow_input.start_time)-Simulator::Now(), ScheduleFlowInputs);
		ScheduleFlowInputs();//added by bo
	}
	...
	Simulator::Stop(Seconds(first_flow_start_time+5));//modified by bo Seconds(simulator_stop_time));
	Simulator::Run();
	Simulator::Destroy();
	NS_LOG_INFO("Done.");
	fclose(trace_output);
