...
namespace ns3 {
	...
	uint32_t SwitchMmu::GetPfcThreshold(uint32_t port){
		...
		//<< "shared used bytes  = " << " " << shared_used_bytes << "\n";
		
		//std::cout << ((buffer_size - total_hdrm - total_rsrv - shared_used_bytes) >> pfc_a_shift[port]);
		return (buffer_size - total_hdrm - total_rsrv - shared_used_bytes) >> pfc_a_shift[port];
		//return(25*1030)-reserve;
	...
	void SwitchMmu::ConfigNPort(uint32_t n_port){
		...
		total_hdrm *= n_port;
		/*std:: cout << "n_port" << " " << n_port << " " << "reserve" << " " << reserve << " " << "total_rsrv" << " "
		<< total_rsrv << " " << "headroom" << " " << total_hdrm << " ";*/
	}
