...
namespace ns3
{
	...
	TypeId RdmaHw::GetTypeId(void)
	{
		static TypeId tid = TypeId("ns3::RdmaHw")
								...
								// tiffany added below attribute for tracking active flows
								.AddAttribute("CheckActiveFlowInterval",
											  "Check if there are active flows",
											  DoubleValue(55.0),
											  MakeDoubleAccessor(&RdmaHw::m_checkactiveflow_interval),
											  MakeDoubleChecker<double>())
								// late notification attributes:
								.AddAttribute("ResetLateCNP",
											  "The interval of resetting lateCNP = 0",
											  DoubleValue(55.0),
											  MakeDoubleAccessor(&RdmaHw::m_resetlateCNP_interval),
											  MakeDoubleChecker<double>())

								.AddAttribute("AdjustVirtualFlowInterval",
											  "The interval of reseting adjustVF=0",
											  DoubleValue(55.0),
											  MakeDoubleAccessor(&RdmaHw::m_adjustVF_interval),
											  MakeDoubleChecker<double>())
								// late notification attributes ended
								// tiffany added ended
	...
	{
		// tiffany added
		m_resetcheckactiveflow = Simulator::Schedule(MicroSeconds(m_checkactiveflow_interval), &RdmaHw::ScheduleCheckActiveFlow, this);
		m_resetlateCNP = Simulator::Schedule(MicroSeconds(m_resetlateCNP_interval), &RdmaHw::resetlateCNP, this);
		// tiffany added create m_VF virtual qp for late notification implementation
		uint16_t pg = 3;
		Ipv4Address sip("0.0.0.0");
		Ipv4Address dip("1.1.1.1");
		uint16_t sport = 100;
		uint16_t dport = 100;
		m_VF = CreateObject<RdmaQueuePair>(pg, sip, dip, sport, dport);
		m_VF->m_rate=DataRate("40Gbps");
		m_VF->mlx.m_targetRate=DataRate("40Gbps");
	...
	void RdmaHw::AddQueuePair(uint64_t size, uint16_t pg, Ipv4Address sip, Ipv4Address dip, uint16_t sport, uint16_t dport, uint32_t win, uint64_t baseRtt, Callback<void> notifyAppFinish)
	{
		...
		// tiffany added: print out the active flows
		uint32_t n_af = checkNumberOfActiveFlow();
		//edit
		//std::cout << Simulator::Now().GetNanoSeconds() << " " << m_node->GetId() << ": exist active flows:" << (n_af > 0 ? 1 : 0) << std::endl;
		// tiffany added ended
		...
		// qp->m_rate = m_bps; // original code

		// // tiffany added assign lowest rate to new flow's initial sending rate
		// if (lowestrate > 0)
		// {
		// 	qp->m_rate = lowestrate;
		// }
		// else
		// 	qp->m_rate = m_bps;
		// // tiffany added assign lowest rate to new flow's initial sending rate ended

		// tiffany added for late notification implementation: assign virtual flow's sending rate (which track lowest sending rate) as new flow's initial sending rate
		
		DataRate cur_lowest = checkActiveFlow(); //add by bo
		
		if (lateCNP == 1)
		{
			//edit x2 313 319
			//std::cout << Simulator::Now().GetNanoSeconds() << " " << m_node->GetId() << " lateCNP " << std::endl;
			if (m_VF->m_rate == 0)
			{
				m_VF->m_rate = m_bps;
			}
			qp->m_rate = m_VF->m_rate;
			//std::cout << Simulator::Now().GetNanoSeconds() << " " << m_node->GetId() << " "
					  //<< "after assigning VF's rate to m_rate " << qp->m_rate << std::endl;
		}
		else
		{
			...
			// by bo
			if ((lowestrate > 0)||(cur_lowest>0))
			{
				if(lowestrate == 0)
					qp->m_rate = cur_lowest;
				else if(cur_lowest == 0)
					qp->m_rate = lowestrate;
				else
					qp->m_rate = std::min(lowestrate, cur_lowest);
			}
			else
				qp->m_rate = m_bps;
			// tiffany added assign lowest rate to new flow's initial sending rate ended
		...
		if (qp->m_rate < m_bps)
		{
			//edit x2 344 and 349
			//std::cout << Simulator::Now().GetNanoSeconds() << " " << m_node->GetId() << " "
					  //<< "new flow initiated lower than link rate " << qp->m_rate << std::endl;
		}
		else
		{
			//std::cout << Simulator::Now().GetNanoSeconds() << " " << m_node->GetId() << " "
					  //<< "new flow initiated at link rate " << qp->m_rate << std::endl;
		}
		// tiffany added for late notification implementation: assign virtual flow's sending rate (which track lowest sending rate) as mew flow's initial sending rate

		qp->m_max_rate = m_bps;
		// // tiffany added print log of initial sending rate
		// std::cout << Simulator::Now().GetNanoSeconds() << " " << m_node->GetId() << " "
		// 		  << "m_rate sending rate " << qp->m_rate << std::endl;
		// // tiffany added end
		if (m_cc_mode == 1)
		{
			// tiffany added assign lowest rate to new flow's initial sending rate
			// if (lowestrate > 0)
			// {
			// 	qp->mlx.m_targetRate = lowestrate;
			// }
			// else
			...
			if (lateCNP == 1)
			{
				if (m_VF->mlx.m_targetRate == 0)
				{
					m_VF->mlx.m_targetRate = m_bps;
				}
				qp->mlx.m_targetRate = m_VF->mlx.m_targetRate;
				//edit
				//std::cout << Simulator::Now().GetNanoSeconds() << " " << m_node->GetId() << " "
						  //<< "after assigning VF's rate to m_targetrate " << qp->mlx.m_targetRate << std::endl;
			}
			// tiffany added for late notification implementation: assign virtual flow's sending rate (which track lowest sending rate) as mew flow's initial sending rate
			else
			{
				...
				if ((lowestrate > 0)||(cur_lowest>0))
				{
					//qp->mlx.m_targetRate = lowestrate;
					if(lowestrate == 0)
						qp->mlx.m_targetRate = cur_lowest;
					else if(cur_lowest == 0)
						qp->mlx.m_targetRate = lowestrate;
					else
						qp->mlx.m_targetRate = std::min(lowestrate, cur_lowest);
			...
			// qp->mlx.m_targetRate = m_bps; // original code
		}
		else if (m_cc_mode == 3)
		{
		...
		//edit 423
		//std::cout << Simulator::Now().GetNanoSeconds() << " " << m_node->GetId() << " done" << std::endl;
		// Notify Nic
	...
	int RdmaHw::ReceiveCnp(Ptr<Packet> p, CustomHeader &ch)
	{
		...
		{	//edit
			//std::cout << Simulator::Now().GetNanoSeconds() << "lazy initialization" << std::endl;
	...
	int RdmaHw::ReceiveAck(Ptr<Packet> p, CustomHeader &ch)
	{
		...
		if (cnp)
		{
			if (m_cc_mode == 1)
			{ // mlx version
				// tiffany added print log of receiving ecn marked cnp

				//edit
				//std::cout << Simulator::Now().GetNanoSeconds() << " " << m_node->GetId() << " "
				//		  << " received ECN " << qp->GetBytesLeft() << std::endl;

				// tiffany added: late notification adjust virtual flow implementation:
				// if sender received late CNP, then lateCNP = 1
				if (qp->GetBytesLeft() == 0)
				{

					if (m_resetlateCNP.IsExpired()) // if m_resetlateCNP is expired for adjusting sending rate, set lateCNP = 1 and start clock again
					{
						lateCNP = 1;
						m_resetlateCNP = Simulator::Schedule(MicroSeconds(m_resetlateCNP_interval), &RdmaHw::resetlateCNP, this);
					}

					else // there is still time left, but recevied new ECN marks, then cancel existing timer and start a new timer.
					{
						Simulator::Cancel(m_resetlateCNP);
						lateCNP = 1;
						m_resetlateCNP = Simulator::Schedule(MicroSeconds(m_resetlateCNP_interval), &RdmaHw::resetlateCNP, this);
					}

					if (adjustVF == 0) // haven't adjusted the VF yet
					{

						if (checkActiveFlow() == 0) // there is no active flow at this moment
						{
							cnp_received_mlx(m_VF); // adjust virtual flow's rate
							//edit
							//std::cout << Simulator::Now().GetNanoSeconds() << " " << m_node->GetId() << " after" << std::endl;
							adjustVF = 1;
							m_resetadjustVF = Simulator::Schedule(MicroSeconds(m_adjustVF_interval), &RdmaHw::resetadjustVF, this);
						}
						else // there are active flows
							m_VF->m_rate = lowestrate;
					}
				}
				else // received normal CNP, usually these are long flows
				{

					cnp_received_mlx(qp); // adjust the current sending rate

					m_VF->m_rate = lowestrate; // assign the lowest rate to the virtual flow
				}
				// tiffany added: late notification adjust virtual flow implementation ended.
				// cnp_received_mlx(qp); original code
			}
		}
...
// std::cout << Simulator::Now() << " alpha update:" << m_node->GetId() << ' ' << q->mlx.m_alpha << ' ' << (int)q->mlx.m_alpha_cnp_arrived << '\n';
 printf("%lu monitor_rate alpha update: %08x %08x %u %u %.6lf->", Simulator::Now().GetTimeStep(), q->sip.Get(), q->dip.Get(), q->sport, q->dport, q->mlx.m_alpha);
#endif
 ...
 printf("%.6lf\n", q->mlx.m_alpha);
		...
		if(q!=m_VF) ScheduleUpdateAlphaMlx(q);
	}
	...
	{
		if(q!=m_VF) ScheduleDecreaseRateMlx(q, 0); //by bo
		if (q->mlx.m_decrease_cnp_arrived)
		{
#if PRINT_LOG
			printf("%lu monitor_rate rate dec: %08x %08x %u %u (%0.3lf %.3lf)->", Simulator::Now().GetTimeStep(), q->sip.Get(), q->dip.Get(), q->sport, q->dport, q->mlx.m_targetRate.GetBitRate() * 1e-9, q->m_rate.GetBitRate() * 1e-9);
#endif
			bool clamp = true;
			...
			if(q!=m_VF) q->mlx.m_rpTimer = Simulator::Schedule(MicroSeconds(m_rpgTimeReset), &RdmaHw::RateIncEventTimerMlx, this, q); //by bo
#if PRINT_LOG
	...
	{
#if PRINT_LOG
		printf("%lu monitor_rate fast recovery: %08x %08x %u %u (%0.3lf %.3lf)->", Simulator::Now().GetTimeStep(), q->sip.Get(), q->dip.Get(), q->sport, q->dport, q->mlx.m_targetRate.GetBitRate() * 1e-9, q->m_rate.GetBitRate() * 1e-9);
#endif
		if(q->mlx.m_targetRate <= m_minRate) q->mlx.m_targetRate=m_minRate*2;//by bo
	...
	{
#if PRINT_LOG
		printf("%lu monitor_rate active inc: %08x %08x %u %u (%0.3lf %.3lf)->", Simulator::Now().GetTimeStep(), q->sip.Get(), q->dip.Get(), q->sport, q->dport, q->mlx.m_targetRate.GetBitRate() * 1e-9, q->m_rate.GetBitRate() * 1e-9);
#endif

		if (q != m_VF) // Tiffany added: since virtual queue pair m_VF does not point to any NIC, we need to check if q is m_VF before find the NIC and GetDAtaRAte
		{
			// get NIC
		...
		else // if q = m_VF
		{
			// increase rate
			q->mlx.m_targetRate += m_rai;
			if (q->mlx.m_targetRate > DataRate("40Gb/s")) // manually compare targetRate with 40Gbps since there is no NIC can be found
				q->mlx.m_targetRate = DataRate("40Gb/s");
		}
		q->m_rate = (q->m_rate / 2) + (q->mlx.m_targetRate / 2);
#if PRINT_LOG
		printf("(%.3lf %.3lf)\n", q->mlx.m_targetRate.GetBitRate() * 1e-9, q->m_rate.GetBitRate() * 1e-9);
	...
	void RdmaHw::HyperIncreaseMlx(Ptr<RdmaQueuePair> q)
	{
		...
		//std::cout << Simulator::Now().GetNanoSeconds() << " " << m_node->GetId() << " "
		//		  << "Trigger HyperIncreaseMlx " << std::endl;

#if PRINT_LOG
		printf("%lu monitor_rate hyper inc: %08x %08x %u %u (%0.3lf %.3lf)->", Simulator::Now().GetTimeStep(), q->sip.Get(), q->dip.Get(), q->sport, q->dport, q->mlx.m_targetRate.GetBitRate() * 1e-9, q->m_rate.GetBitRate() * 1e-9);
#endif
		if (q != m_VF) // Tiffany added: since virtual queue pair m_VF does not point to any NIC, we need to check if q is m_VF before find the NIC and GetDAtaRAte
		{
			// get NIC
	...
	DataRate RdmaHw::checkActiveFlow() // tiffany added: check if there are active flows then get the current lowest rate
	{
		DataRate ret = 0;
	...
	void RdmaHw::ScheduleCheckActiveFlow() // tiffany added
	{
		// std::cout << Simulator::Now().GetNanoSeconds() << "scheduleCheckActiveFLow function" << std::endl;
		if (m_node->GetNodeType())
		{
			Simulator::Cancel(m_resetcheckactiveflow);
			//edit
			//std::cout << Simulator::Now().GetNanoSeconds() << " node type in resetcheckactiveflow " << m_node->GetNodeType() << std::endl;
			return;
		}
		lowestrate = checkActiveFlow();
		if (lowestrate == 0)
		{	//edit
			//std::cout << Simulator::Now().GetNanoSeconds() << " resetCheckActiveFlow " << m_node->GetId() << " no active flows" << std::endl;

			// Ptr<QbbNetDevice> dev = m_nic[0].dev;
			lowestrate = DataRate("40Gb/s");
		}
		else
		{	//edit
			//std::cout << Simulator::Now().GetNanoSeconds() << " resetCheckActiveFlow " << m_node->GetId() << " active flows lowestrate is: " << lowestrate << std::endl;
		}
		Simulator::Cancel(m_resetcheckactiveflow);
		m_resetcheckactiveflow = Simulator::Schedule(MicroSeconds(m_checkactiveflow_interval), &RdmaHw::ScheduleCheckActiveFlow, this);
	}
	uint32_t RdmaHw::checkNumberOfActiveFlow() // tiffany added: check number of active flows
	{
		uint32_t ret = 0;
		for (auto &it : m_qpMap)
		{
			...
			if (q->GetBytesLeft() > 0)
			{
				if (q->dip.Get() == 0x0b008001) // only counts write flows, dip = 0b008001
				{
					ret++;
				}
			}
	...
	void RdmaHw::resetlateCNP() // tiffany added: reset lateCNP from 1 to 0 for late notification implementation;
	{
		lateCNP = 0;
		//edit
		//std::cout << Simulator::Now().GetNanoSeconds() << " " << m_node->GetId() << " resetlateCNP " << std::endl;
	}
	void RdmaHw::resetadjustVF() // tiffany added: reset adjustVF from 1 to 0 for late notification implementation;
	{
		adjustVF = 0;
		//edit
		//std::cout << Simulator::Now().GetNanoSeconds() << " " << m_node->GetId() << " resetadjustVF " << std::endl;
	}
