...
namespace ns3
{
	...
	public:
		 ...
		 *****************************/
		// tiffany added variables
		// tradk lowest rate
		DataRate lowestrate;
		double m_checkactiveflow_interval;
		EventId m_resetcheckactiveflow;
		uint32_t checkNumberOfActiveFlow();
		void ScheduleCheckActiveFlow();
		DataRate checkActiveFlow();

		// late notification:
		uint32_t lateCNP;
		uint32_t adjustVF;
		double m_adjustVF_interval;
		double m_resetlateCNP_interval;
		EventId m_resetlateCNP;
		EventId m_resetadjustVF;
		void resetlateCNP();
		void resetadjustVF();
		Ptr<RdmaQueuePair> m_VF;
